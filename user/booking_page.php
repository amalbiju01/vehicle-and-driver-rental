...
<div class="container">
    ...
    <h2 class="text-warning">Number of Days: <span id="numOfDays">0</span></h2>
            <h2 class="text-warning">Cost Per Day: Rs <span id="costPerDay"><?php echo $row['price']; ?></span></h2>
...
if (isset($_REQUEST['book'])) {
    ...
    $qry = "SELECT * FROM `booking` WHERE `s_date` = '$startDate' AND e_date='$endDate'AND `bid`='$id'";
	// echo $qry;
    ...
    $sql = "INSERT INTO `booking` (`s_date`, `e_date`,`bid`,`rate`,`uid`)
    VALUES ('$startDate', '$endDate', '$id','$rate','$uid')";
        mysqli_query($con, $sql);
        $query = "UPDATE vehicle  SET `status`='booked' WHERE `bid`='$id'";
        $result = mysqli_query($con, $query);


        echo "<script>alert('Booked Successfully');</script>";
        echo "<script>window.location='./user_view_veh.php'</script>";
